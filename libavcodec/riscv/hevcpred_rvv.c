...
static av_always_inline void *MEMCPY_RVV(void *restrict destination,
...
static av_always_inline void *MEMSET_RVV(void *destination, int ch, size_t n)
{
	unsigned char *dst = destination;
	size_t vlmax = vsetvlmax_e8m1();
	vuint8m1_t vec_ch = vmv_v_x_u8m1((unsigned char)ch, vlmax);
	// set data byte by byte
	for (size_t vl; n > 0; n -= vl, dst += vl) {
		vl = vsetvl_e8m1(n);
		vse8_v_u8m1(dst, vec_ch, vl);
	}
	return destination;
}

static av_always_inline void intra_pred_rvv(HEVCContext *s, int x0, int y0,
...
#define EXTEND(ptr, val, len)    MEMSET_RVV(ptr, val, len)
...
#define EXTEND_DOWN_CIP(ptr, start, length)                                    \
    ...
    if (s->ps.pps->constrained_intra_pred_flag == 1) {
        ...
        MEMSET_RVV(left, 128, 2 * MAX_TB_SIZE*sizeof(pixel));
        MEMSET_RVV(top , 128, 2 * MAX_TB_SIZE*sizeof(pixel));
        top[-1] = 128;
    }
    if (cand_up_left) {
        left[-1] = POS(-1, -1);
    ...
    if (cand_up)
        MEMCPY_RVV(top, src - stride, size * sizeof(pixel));
    if (cand_up_right) {
        MEMCPY_RVV(top + size, src - stride + size, size * sizeof(pixel));
...
void intra_pred_##size##_rvv(HEVCContext *s, int x0, int y0, int c_idx)    \
{                                                                          \
    intra_pred_rvv(s, x0, y0, size, c_idx);                                \
}
